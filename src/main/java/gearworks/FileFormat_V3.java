...
public class FileFormat_V3 extends FileFormat {
    ...
    final int LENGTH_OF_LINE_ITEM = 5;
	final int SPEC_NUMBER_OFFSET = 0;
    final int LINE_ITEM_DESCRIPTION_OFFSET = 1;
	final int UNIT_OFFSET = 2;
    final int LINE_ITEM_QUANTITY_OFFSET = 3;
    final int LINE_ITEM_PRICE_OFFSET = 4;
     ...
     * Format.
     */
    public static final String fileHeader = "----V3 FORMAT----";

    /**
     * Default constructor for V2Format.
     */
    public FileFormat_V3() {
    ...
    public String jobToFormat(Job job) {
        ...
        for (int lineItem = 0; lineItem < LINE_ITEM_COUNT; lineItem++) {
			...
			result.add(starOfThisLineItem + SPEC_NUMBER_OFFSET, 
					lineItems.get(lineItem).getSpecNumber());
					
            result.add(starOfThisLineItem + LINE_ITEM_DESCRIPTION_OFFSET,
                    lineItems.get(lineItem).getDescription());

			result.add(starOfThisLineItem + UNIT_OFFSET,
                    lineItems.get(lineItem).getUnit());
    ...
    public Job jobFromFormat(String jobLineString) {
        ...
        for (int lineItem = 0; lineItem < LINE_ITEM_COUNT; lineItem++) {
            ...
            lineItems.add(new LineItem(
					tokens[starOfThisLineItem + SPEC_NUMBER_OFFSET],
                    tokens[starOfThisLineItem + LINE_ITEM_DESCRIPTION_OFFSET],
					tokens[starOfThisLineItem + UNIT_OFFSET],
